    ...
    pub fn update(&mut self, delta: Duration) -> bool {
        ...
        if self.move_timer.ready {
            ...
            if self.direction == -1 {
                ...
                if min_x == 0 { // Reached end of row, go down and change directions
                    self.direction = 1;
                    downwards = true;
                }
            } else {
                let max_x = self.army.iter().map(|invader| invader.x).max().unwrap_or(0);
                if max_x == NUM_COLS - 1 { // Reached end of row, go down and change directions
                    self.direction = -1;
                    downwards = true;
                }
            }
