...
fn main() {
    ...
    println!("=== Challenge Start! ===");

    let (c_tx, c_rx) = channel::unbounded();
    let c_rx2 = c_rx.clone();

    let c_handle_a = thread::spawn(|| {
        for msg in c_rx{
            println!("Thread A: Received {}", msg);
        }
    });

    let c_handle_b = thread::spawn(|| {
        for msg in c_rx2{
    ...
    for i in 1..10 {
        c_tx.send(format!("{}", i)).unwrap();
        pause_ms(200);
    }

    drop(c_tx);

    c_handle_a.join().unwrap();
    c_handle_b.join().unwrap();

    println!("=== Challenge ended ===");
}
